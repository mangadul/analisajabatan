...
Ext.onReady(function(){
    ...
    Ext.define('mdl_temperamen', {
        extend: 'Ext.data.Model',
        fields: [ "id", "kode_rumpun", "kode","nama","id_jens_jabatan","jenis_jabatan","jenis_jabatan2"],
        idProperty: 'id'
    });

    var store_temperamen = Ext.create('Ext.data.Store', {
        pageSize: 200,
        model: 'mdl_temperamen',
        remoteSort: true,
        proxy: {
	...
	var ce_temperamen = Ext.create('Ext.grid.plugin.RowEditing', {
		//clicksToEdit: 1,
		clicksToMoveEditor: 1,
		autoCancel: false,
		listeners : {
			'edit' : function() {						
				var editedRecords = grid_temperamen.getView().getSelectionModel().getSelection();
				Ext.Ajax.request({
...
var grid_temperamen = Ext.create('Ext.grid.Panel', {
	title: 'JABATAN',
	store: store_temperamen,
	...
	*/
	...
	dockedItems: [
	{
		...
		[
		{
			...
			iconCls: 'icon-add',
			handler: function(){          
				var r = Ext.create('mdl_temperamen', {
					//id: null,
					kode: '[KODE]',
					nama: '[NAMA JABATAN]',
					id_jens_jabatan: 1,
				});
				store_temperamen.insert(0, r);
				ce_temperamen.startEdit(0, 0);									
			}
		},
		{
			text:'Delete',
			iconCls: 'icon-del',
			handler: function() {          
				var records = grid_temperamen.getView().getSelectionModel().getSelection(), id=[];
		...
		'->',
		{
			...
			listeners: {
				'select': function(combo, row, index) {
					var id = row[0].get('id');
					store_temperamen.load({params:{'id_jens_jabatan':id}});
				}
			},																							
		},
		{
