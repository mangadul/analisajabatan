...
Ext.onReady(function(){
    ...
    Ext.define('mdl13', {
        ...
        fields: [ "id", "id_kecamatan","kecamatan","kelurahan"],
        idProperty: 'id'
    });
    ...
    var store13 = Ext.create('Ext.data.Store', {
        ...
        remoteSort: true,
        proxy: {
            url: '<?=base_url()?>index.php/master_data/main/get_data_kelurahan',
            simpleSortMode: true,
			type: 'ajax',
			reader: {
				type: 'json',
	...
	var APcellEditing13 = Ext.create('Ext.grid.plugin.RowEditing', {
		...
		listeners : {
			'edit' : function() {						
				...
				Ext.Ajax.request({
					url: '<?=base_url();?>index.php/master_data/main/simpan_data_kelurahan',
					method: 'POST',
					params: {
						'id' : editedRecords[0].data.id,
						'id_kecamatan' : editedRecords[0].data.id_kecamatan,
						'kelurahan': editedRecords[0].data.kelurahan,
					},								
		...
		},
		{
			...
			handler: function() {          
				...
				function(resbtn){
					...
					{
						Ext.Ajax.request({
							url: '<?=base_url();?>index.php/master_data/main/hapus_data_kelurahan',
							method: 'POST',											
							params: {												
								'id' : id.join(','),
							},								
