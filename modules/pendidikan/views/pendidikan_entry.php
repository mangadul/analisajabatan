...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url:  '<?=base_url()?>index.php/pendidikan/main/DataListpendidikan',
            reader: {
                root: 'pendidikan_data',
    ...
    function SaveData(form, win) {
        var pendidikan =  form.query('textfield[name="pendidikan"]')[0].getValue('pendidikan');
        
        var conn = new Ext.data.Connection();
        conn.request({
            method: 'POST',
            url: '<?=base_url()?>index.php/pendidikan/main/Insertpendidikan',
            params: {
                pendidikan : pendidikan
            },
            success: function() {
                Ext.example.msg('Sukses', 'Data pendidikan sudah tersimpan.');
                form.getForm().reset();
                store.loadPage(1);
                win.hide();
            }
    ...
    function Formpendidikan(tag){
        ...
        if (tag){
			...
			form2.findField('pendidikan').setValue(rec.get('pendidikan'));
        } else {
            form2.reset();
    ...
    var grid = Ext.create('Ext.grid.Panel', {
        ...
        dockedItems: [{
            ...
            items: [
            {
                ...
                iconCls: 'icon-remove',
                scope: this,
                handler: function(){Hapuspendidikan()}
            },{
                text: 'UPDATE',
                iconCls: 'icon-save',
                scope: this,
    ...
    function Hapuspendidikan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan menghapus pendidikan : ' + rec.get('pendidikan') ,function(btn) {
                if (btn == 'yes') {
                   
                   $.post("<?=base_url()?>index.php/pendidikan/main/Hapuspendidikan", 
                   {'id':rec.get('id')},
    ...
    function Updatependidikan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan mengupdate pendidikan : ' + rec.get('pendidikan') ,function(btn) {
                if (btn == 'yes') {
                   $.post("<?=base_url()?>index.php/pendidikan/main/Updatependidikan", 
                   {'id': id, 'pendidikan': pendidikan},
                   function(data){$('#jqueryExec').html(data)}); 
                    form.getForm().reset();
					store.loadPage(1);
					win.hide();
