...
class mod_agama extends CI_Model {
    ...
    function MasterAgama($q, $start, $limit){
        $sql = "SELECT SQL_CALC_FOUND_ROWS 
                *
                FROM m_agama
                WHERE(id LIKE '%$q%' OR agama LIKE '%$q%')
                ORDER by id
                LIMIT $start,$limit";    
    ...
    function HapusAgama($id) {
        $this->db->delete('m_agama',array('id'=>$id));
    }
	...
	function UpdateAgama($id, $agama) {
		$sql = "UPDATE m_agama SET agama = '$agama' WHERE id='$id'"; 
		$this->db->query($sql);
	}
}
