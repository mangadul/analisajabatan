...
class mod_kursus extends CI_Model {
    function __construct()
    {
        parent::__construct();
    }
    
    function Masterkursus($q, $start, $limit){
        $sql = "SELECT SQL_CALC_FOUND_ROWS 
                *
                FROM m_kursus
                WHERE(id LIKE '%$q%' OR kursus LIKE '%$q%')
                ORDER by id
                LIMIT $start,$limit";    
    ...
    function Hapuskursus($id) {
        $this->db->delete('m_kursus',array('id'=>$id));
    }
    ...
    function Insertkursus($kursus) {        
        $data = array(
            'kursus' => $kursus
        );
		
        $this->db->insert('m_kursus', $data);
    }
	
	function Updatekursus($id, $kursus) {
		$sql = "UPDATE m_kursus SET kursus = '$kursus' WHERE id='$id'"; 
		$this->db->query($sql);
	}
}
