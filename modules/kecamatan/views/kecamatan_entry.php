...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url:  '<?=base_url()?>index.php/kecamatan/main/DataListKecamatan',
            reader: {
                root: 'nama_data',
    ...
    function SaveData(form, win) {
        var kecamatan =  form.query('textfield[name="nama"]')[0].getValue('nama');
        
        var conn = new Ext.data.Connection();
        conn.request({
            method: 'POST',
            url: '<?=base_url()?>index.php/kecamatan/main/InsertKecamatan',
            params: {
                nama : nama
            },
            success: function() {
                Ext.example.msg('Sukses', 'Data kecamatan sudah tersimpan.');
                form.getForm().reset();
                store.loadPage(1);
                win.hide();
            }
    ...
    var grid = Ext.create('Ext.grid.Panel', {
        ...
        dockedItems: [{
            ...
            items: [
            {
                ...
                iconCls: 'icon-remove',
                scope: this,
                handler: function(){HapusKecamatan()}
            },{
                text: 'UPDATE',
                iconCls: 'icon-save',
                scope: this,
    ...
    function HapusKecamatan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan menghapus Kecamatan : ' + rec.get('nama') ,function(btn) {
                if (btn == 'yes') {
                   
                   $.post("<?=base_url()?>index.php/kecamatan/main/HapusKecamatan", 
                   {'id':rec.get('id')},
    ...
    function UpdateKecamatan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan mengupdate Kecamatan : ' + rec.get('nama') ,function(btn) {
                if (btn == 'yes') {
                   $.post("<?=base_url()?>index.php/kecamatan/main/UpdateKecamatan", 
                   {'id': id, 'nama': nama},
                   function(data){$('#jqueryExec').html(data)}); 
                    form.getForm().reset();
					store.loadPage(1);
