...
Ext.onReady(function(){
    ...
    Ext.define('mdl11', {
        ...
        fields: [ "id", "kode_jabatan", "wewenang"],
        idProperty: 'id'
    });
    ...
    var store11 = Ext.create('Ext.data.Store', {
        ...
        remoteSort: true,
        proxy: {
            url: '<?=base_url()?>index.php/entry_sdm/main/get_wewenang',
            simpleSortMode: true,
			type: 'ajax',
			reader: {
				type: 'json',
	...
	var APcellEditing11 = Ext.create('Ext.grid.plugin.RowEditing', {
		clicksToMoveEditor: 1,
		autoCancel: false,
		listeners : {
			'edit' : function() {						
				var editedRecords = grid11.getView().getSelectionModel().getSelection();
				Ext.Ajax.request({
					url: '<?=base_url();?>index.php/entry_sdm/main/simpan_wewenang',
					method: 'POST',
					params: {
						'id' : editedRecords[0].data.id,
						'kode_jabatan': editedRecords[0].data.kode_jabatan,
						'wewenang': editedRecords[0].data.wewenang,
					},								
...
var grid11 = Ext.create('Ext.grid.Panel', {
	...
	dockedItems: [
	{
		...
		[
		{
			...
			iconCls: 'icon-add',
			handler: function(){          
				var r = Ext.create('mdl11', {
					//id: null,
					kode_jabatan: '<?=$data['kode']?>',
					wewenang: '[ EDITED - WEWENANG]',
				});
				store11.insert(0, r);
				APcellEditing11.startEdit(0, 0);									
			}
		},
		{
			text:'Delete',
			iconCls: 'icon-del',
			handler: function() {          
				var records = grid11.getView().getSelectionModel().getSelection(), id=[];
				...
				function(resbtn){
					...
					{
						Ext.Ajax.request({
							url: '<?=base_url();?>index.php/entry_sdm/main/hapus_wewenang',
							method: 'POST',											
							params: {												
								'id' : id.join(','),
							},								
