...
Ext.onReady(function(){
    ...
    Ext.define('mdl13', {
        ...
        fields: [ "id", "kode_jabatan", "aspek", "faktor"],
        idProperty: 'id'
    });
    ...
    var store13 = Ext.create('Ext.data.Store', {
        ...
        remoteSort: true,
        proxy: {
            url: '<?=base_url()?>index.php/entry_sdm/main/get_lingkungan_kerja',
            simpleSortMode: true,
			type: 'ajax',
			reader: {
				type: 'json',
	...
	var APcellEditing13 = Ext.create('Ext.grid.plugin.RowEditing', {
		...
		listeners : {
			'edit' : function() {						
				...
				Ext.Ajax.request({
					url: '<?=base_url();?>index.php/entry_sdm/main/simpan_lingkungan_kerja',
					method: 'POST',
					params: {
						'id' : editedRecords[0].data.id,
						'kode_jabatan': editedRecords[0].data.kode_jabatan,
						'aspek': editedRecords[0].data.aspek,
						'faktor': editedRecords[0].data.faktor,
					},								
		...
		},
		{
			...
			handler: function() {          
				...
				function(resbtn){
					...
					{
						Ext.Ajax.request({
							url: '<?=base_url();?>index.php/entry_sdm/main/hapus_lingkungan_kerja',
							method: 'POST',											
							params: {												
								'id' : id.join(','),
							},								
