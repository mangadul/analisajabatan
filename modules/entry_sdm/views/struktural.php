...
Ext.onReady(function(){
    ...
    var storestruk = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            url: '<?=base_url()?>index.php/entry_sdm/main/get_jabatan_all',
            simpleSortMode: true,
			type: 'ajax',
			reader: {
				type: 'json',
			...
			cellclick: function(view, td, cellIndex, record, tr, rowIndex, e, eOpts) {
				...
				Ext.Ajax.request({
					...
					success : function(response, opts) {
                        ...
                        Ext.getCmp('kd_instansi').update('Instansi: '+result.nama_instansi);
                        var panel = Ext.getCmp('diagram');
                        var tabform = Ext.getCmp('tbisian');
                        if(result.msg == 'ok')
                        {
                            if(result.success)
                            {
                                panel.setDisabled(false);
                                tabform.setDisabled(false);
                                Ext.getCmp('tb-print').setDisabled(false);
                                Ext.getCmp('tb-pilih-instansi').setDisabled(true);
                                tabform.setActiveTab(0);
                                panel.body.update();
                                panel.doLayout();
                            }
                        } else {
                            panel.setDisabled(false);
                            Ext.getCmp('tb-pilih-instansi').setDisabled(false);
                            Ext.getCmp('tb-print').setDisabled(true);
                            Ext.getCmp('stat_instansi').setDisabled(false);
                            Ext.getCmp('tbisian').setDisabled(true);
                            //var pilih_inst = Ext.getCmp('cmb_pilih_instansi');                            
                            //pilih_inst.store.reload();
                            //console.log(pilih_inst.store);
                            tabform.setActiveTab(0);
                            panel.body.update();
                            panel.doLayout();
                        }
...
*/
    ...
    var pp = Ext.create('Ext.panel.Panel', {
        ...
        items:[
            {
                html: "<iframe id='ifpp' height='100%' width='100%' src='<?=$this->config->item('base_url');?>index.php/entry_sdm/isian_jabatan/cetak_jabatan/'></iframe>"
            }
        /*
        {
        xtype : "component",
        autoEl:{
            tag:'iframe',
            id: 'ifp',
            src :'<?=$this->config->item('base_url');?>index.php/entry_sdm/isian_jabatan/cetak_jabatan/'
        }
        }
        */
        ]
    ...
    var win_preview = Ext.create('Ext.window.Window',{
        layout: 'fit',        
        items: [pp],
        closeAction: 'hide',
        closable: true,
        autoScroll: true,
        title: 'Preview Formulir Jabatan',
        width: '80%',
        height: '80%',
        modal: true,
        dockedItems: [
                ...
                items: [                
                    {
                        ...
                        handler: function()
                        {
                            window.open('<?php echo base_url();?>index.php/entry_sdm/isian_jabatan/cetak_jabatan/print/1','_blank');
                        },
                    },
                    {
                        text: 'Download',
                        handler: function()
                        {
                            window.open('<?php echo base_url();?>index.php/entry_sdm/isian_jabatan/download_odt','_blank');
                        },
                    },
                    {
                        text: 'Refresh',
                        handler: function()
                        {
                            document.getElementById('ifpp').contentWindow.location.reload()
                        }
                    },                    
                ]
            },
                    ...
                    items: [
                        {
                            ...
                            listeners: {
                                'select': function(combo, row, index) {
                                    Ext.Ajax.request({
                                        url: '<?=base_url();?>index.php/entry_sdm/isian_jabatan/pilih_instansi',
                                        method: 'POST',
                                        params: {
                                            'kode_instansi' : row[0].get('kode'),
                                            'nama_instansi' : row[0].get('nama')
                                        },                              
                                        success: function(response) {
                                            var text = response.responseText;
                                            Ext.MessageBox.alert( "Status", text, function(){
                                                Ext.getCmp('kd_instansi').update('Instansi: '+row[0].get('kode')+' :: '+row[0].get('nama'));
                                                var panel = Ext.getCmp('diagram');
                                                var tabform = Ext.getCmp('tbisian');                                
                                                panel.setDisabled(false);                                
                                                tabform.setDisabled(false);
					...
					items: [
						{
							...
							id: 'print_preview',
                            handler: function(){
                                win_preview.on('show', function(win) {
                                });
                                win_preview.doLayout();                        
                                win_preview.show();                            
                            }
						},'-',
						{
