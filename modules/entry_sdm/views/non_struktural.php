...
Ext.onReady(function(){
    ...
    var storestruk = Ext.create('Ext.data.Store', {
        ...
        remoteSort: true,
        proxy: {
            url: '<?=base_url()?>index.php/entry_sdm/main/get_jabatan_non_struktural',
            simpleSortMode: true,
			type: 'ajax',
			reader: {
				type: 'json',
    ...
    var grid = Ext.create('Ext.grid.Panel', {
		...
		listeners: {
			cellclick: function(view, td, cellIndex, record, tr, rowIndex, e, eOpts) {
				...
				IDJab = record.get('id');
				Ext.Ajax.request({
					url : '<?=$this->config->item('base_url');?>index.php/entry_sdm/isian_jabatan/set_id_jabatan_non_struk/'+IDJab,
					method : "POST",
					...
					items: [
						{
							...
							listeners: {
								'select': function(combo, row, index) {
									Ext.Ajax.request({
										url: '<?=base_url();?>index.php/entry_sdm/main/set_kode_instansi',
										method: 'POST',
										params: {
