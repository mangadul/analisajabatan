...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url:  '<?=base_url()?>index.php/eselon/main/DataListeselon',
            reader: {
                root: 'eselon_data',
    ...
    function SaveData(form, win) {
        var eselon =  form.query('textfield[name="eselon"]')[0].getValue('eselon');
        
        var conn = new Ext.data.Connection();
        conn.request({
            method: 'POST',
            url: '<?=base_url()?>index.php/eselon/main/Inserteselon',
            params: {
                eselon : eselon
            },
            success: function() {
                Ext.example.msg('Sukses', 'Data eselon sudah tersimpan.');
                form.getForm().reset();
                store.loadPage(1);
                win.hide();
            }
    ...
    function Formeselon(tag){
        ...
        if (tag){
			...
			form2.findField('eselon').setValue(rec.get('eselon'));
        } else {
            form2.reset();
    ...
    function Hapuseselon(){
        var rec = grid.getSelectionModel().getSelection()[0]; 
        
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan menghapus eselon : ' + rec.get('eselon') ,function(btn) {
                if (btn == 'yes') {
                   
                   $.post("<?=base_url()?>index.php/eselon/main/Hapuseselon", 
                   {'id':rec.get('id')},
    ...
    function Updateeselon(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan mengupdate eselon : ' + rec.get('eselon') ,function(btn) {
                if (btn == 'yes') {
                   $.post("<?=base_url()?>index.php/eselon/main/Updateeselon", 
                   {'id': id, 'eselon': eselon},
                   function(data){$('#jqueryExec').html(data)}); 
                    form.getForm().reset();
					store.loadPage(1);
