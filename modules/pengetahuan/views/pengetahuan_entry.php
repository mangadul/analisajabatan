...
Ext.onReady(function(){
    ...
    Ext.define('model_pengetahuan', {
        extend: 'Ext.data.Model',
        fields: ['id', 'pengetahuan']
    });
    

    var store = Ext.create('Ext.data.Store', {
        pageSize: 50,
        model: 'model_pengetahuan',
        proxy: {
            type: 'ajax',
            url:  '<?=base_url()?>index.php/pengetahuan/main/DataListpengetahuan',
            reader: {
                root: 'pengetahuan_data',
    ...
    function SaveData(form, win) {
        var pengetahuan =  form.query('textfield[name="pengetahuan"]')[0].getValue('pengetahuan');
        
        var conn = new Ext.data.Connection();
        conn.request({
            method: 'POST',
            url: '<?=base_url()?>index.php/pengetahuan/main/Insertpengetahuan',
            params: {
                pengetahuan : pengetahuan
            },
            success: function() {
                Ext.example.msg('Sukses', 'Data pengetahuan sudah tersimpan.');
                form.getForm().reset();
                store.loadPage(1);
                win.hide();
            }
    ...
    function Formpengetahuan(tag){
        ...
        if (tag){
			...
			form2.findField('pengetahuan').setValue(rec.get('pengetahuan'));
        } else {
            form2.reset();
    ...
    var grid = Ext.create('Ext.grid.Panel', {
        ...
        dockedItems: [{
            ...
            items: [
            {
                ...
                iconCls: 'icon-remove',
                scope: this,
                handler: function(){Hapuspengetahuan()}
            },{
                text: 'UPDATE',
                iconCls: 'icon-save',
                scope: this,
                handler: function(){
                    ...
                    var rec = grid.getSelectionModel().getSelection()[0];
                    var id = rec.get('id');
					//var pengetahuan = rec.get('pengetahuan');
                    Formpengetahuan(1);                        
                }                
            },'->','CARI : ',{
                width: 250,
    ...
    function Hapuspengetahuan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan menghapus pengetahuan : ' + rec.get('pengetahuan') ,function(btn) {
                if (btn == 'yes') {
                   
                   $.post("<?=base_url()?>index.php/pengetahuan/main/Hapuspengetahuan", 
                   {'id':rec.get('id')},
    ...
    function Updatepengetahuan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan mengupdate pengetahuan : ' + rec.get('pengetahuan') ,function(btn) {
                if (btn == 'yes') {
                   $.post("<?=base_url()?>index.php/pengetahuan/main/Updatepengetahuan", 
                   {'id': id, 'pengetahuan': pengetahuan},
                   function(data){$('#jqueryExec').html(data)}); 
                    form.getForm().reset();
					store.loadPage(1);
					win.hide();
