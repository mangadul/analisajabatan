...
class mod_pengetahuan extends CI_Model {
    function __construct()
    {
        parent::__construct();
    }
    
    function Masterpengetahuan($q, $start, $limit){
        $sql = "SELECT SQL_CALC_FOUND_ROWS 
                *
                FROM m_pengetahuan
                WHERE(id LIKE '%$q%' OR pengetahuan LIKE '%$q%')
                ORDER by id
                LIMIT $start,$limit";    
    ...
    function Hapuspengetahuan($id) {
        $this->db->delete('m_pengetahuan',array('id'=>$id));
    }
    ...
    function Insertpengetahuan($pengetahuan) {        
        $data = array(
            'pengetahuan' => $pengetahuan
        );
		
        $this->db->insert('m_pengetahuan', $data);
    }
	
	function Updatepengetahuan($id, $pengetahuan) {
		$sql = "UPDATE m_pengetahuan SET pengetahuan = '$pengetahuan' WHERE id='$id'"; 
		$this->db->query($sql);
	}
}
