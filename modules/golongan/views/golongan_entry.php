...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        pageSize: 50,
        model: 'model_golongan',
        proxy: {
            type: 'ajax',
            url:  '<?=base_url()?>index.php/golongan/main/DataListgolongan',
            reader: {
                root: 'golongan_data',
    ...
    function SaveData(form, win) {
        var golongan =  form.query('textfield[name="golongan"]')[0].getValue('golongan');
        
        var conn = new Ext.data.Connection();
        conn.request({
            method: 'POST',
            url: '<?=base_url()?>index.php/golongan/main/Insertgolongan',
            params: {
                golongan : golongan
            },
            success: function() {
                Ext.example.msg('Sukses', 'Data golongan sudah tersimpan.');
                form.getForm().reset();
                store.loadPage(1);
                win.hide();
            }
    ...
    function Formgolongan(tag){
        ...
        if (tag){
			...
			form2.findField('golongan').setValue(rec.get('golongan'));
        } else {
            form2.reset();
    ...
    var grid = Ext.create('Ext.grid.Panel', {
        ...
        dockedItems: [{
            ...
            items: [
            {
                ...
                iconCls: 'icon-remove',
                scope: this,
                handler: function(){Hapusgolongan()}
            },{
                text: 'UPDATE',
                iconCls: 'icon-save',
                scope: this,
    ...
    function Hapusgolongan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan menghapus golongan : ' + rec.get('golongan') ,function(btn) {
                if (btn == 'yes') {
                   
                   $.post("<?=base_url()?>index.php/golongan/main/Hapusgolongan", 
                   {'id':rec.get('id')},
    ...
    function Updategolongan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan mengupdate golongan : ' + rec.get('golongan') ,function(btn) {
                if (btn == 'yes') {
                   $.post("<?=base_url()?>index.php/golongan/main/Updategolongan", 
                   {'id': id, 'golongan': golongan},
                   function(data){$('#jqueryExec').html(data)}); 
                    form.getForm().reset();
					store.loadPage(1);
					win.hide();
