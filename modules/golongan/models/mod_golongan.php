...
class mod_golongan extends CI_Model {
    function __construct()
    {
        parent::__construct();
    }
    
    function Mastergolongan($q, $start, $limit){
        $sql = "SELECT SQL_CALC_FOUND_ROWS 
                *
                FROM m_golongan
                WHERE(id LIKE '%$q%' OR golongan LIKE '%$q%')
                ORDER by id
                LIMIT $start,$limit";    
    ...
    function Hapusgolongan($id) {
        $this->db->delete('m_golongan',array('id'=>$id));
    }
    ...
    function Insertgolongan($golongan) {        
        $data = array(
            'golongan' => $golongan
        );
	...
	function Updategolongan($id, $golongan) {
		$sql = "UPDATE m_golongan SET golongan = '$golongan' WHERE id='$id'"; 
		$this->db->query($sql);
	}
}
