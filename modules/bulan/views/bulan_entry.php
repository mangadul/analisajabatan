...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url:  '<?=base_url()?>index.php/bulan/main/DataListbulan',
            reader: {
                root: 'bulan_data',
    ...
    function SaveData(form, win) {
        var bulan =  form.query('textfield[name="bulan"]')[0].getValue('bulan');
        
        var conn = new Ext.data.Connection();
        conn.request({
            method: 'POST',
            url: '<?=base_url()?>index.php/bulan/main/Insertbulan',
            params: {
                bulan : bulan
            },
            success: function() {
                Ext.example.msg('Sukses', 'Data bulan sudah tersimpan.');
                form.getForm().reset();
                store.loadPage(1);
                win.hide();
            }
    ...
    function Formbulan(tag){
        ...
        if (tag){
			...
			form2.findField('bulan').setValue(rec.get('bulan'));
        } else {
            form2.reset();
    ...
    function Hapusbulan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan menghapus bulan : ' + rec.get('bulan') ,function(btn) {
                if (btn == 'yes') {
                   
                   $.post("<?=base_url()?>index.php/bulan/main/Hapusbulan", 
                   {'id':rec.get('id')},
    ...
    function Updatebulan(){
        ...
        if (rec.get('id') != '') {
            Ext.MessageBox.confirm('Konfirmasi', 'Apakah Anda akan mengupdate bulan : ' + rec.get('bulan') ,function(btn) {
                if (btn == 'yes') {
                   $.post("<?=base_url()?>index.php/bulan/main/Updatebulan", 
                   {'id': id, 'bulan': bulan},
                   function(data){$('#jqueryExec').html(data)}); 
                    form.getForm().reset();
					store.loadPage(1);
